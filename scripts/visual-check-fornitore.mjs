...
await page.waitForLoadState("networkidle");
  await registraEEntra(page, sql, { base: BASE, nome: "Silvia Marino", email: email, pwd: "PasswordSicura123!" });
...
const card = page.locator('[data-slot="card"]').filter({ hasText: "Carpenteria Sarnese S.r.l." }).first();
await card.waitFor({ timeout: 20000 });
const link = card.getByRole("link", { name: "Fornitore", exact: true });
